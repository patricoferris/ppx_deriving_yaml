...
let enabled_versions =
  [ ("recursive", "(enabled_if (>= %{ocaml_version} \"5.2.0\"))") ]
    ...
    if not expect_failure then
      Format.fprintf ppf
        "; The PPX-dependent executable under test@,\
         ...
         ppx_deriving_ezjsonm))@ (libraries ezjsonm) %s)@]"
  ...
  let pp_rule ppf base =
    ...
    Format.fprintf ppf
      "; Run the PPX on the `.ml` file@,\
       ...
       @[<v 1>(action@,\
       %a))@]@]"
      base (lookup_enabled base) base pp_action expect_failure
  in
